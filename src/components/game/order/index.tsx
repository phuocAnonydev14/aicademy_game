...
export default function WordPhysicsGame() {
  ...
  const startLevel = (levelNum: number) => {
    ...
    const letters = newTargetWord.sort(() => Math.random() - 0.5);
    const blockSize = 80;
    ...
    letters.forEach((letter, index) => {
      ...
      Matter.Events.on(renderRef.current, 'afterRender', () => {
        ...
        if (
          letterBlock.id &&
          Matter.Composite.get(engineRef.current!.world, letterBlock.id, 'body')
        ) {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-3xl mx-auto p-4">
      ...
      <div
        ref={sceneRef}
        className="w-full h-[500px] border border-gray-300 rounded-lg mb-4 overflow-hidden"
      />
